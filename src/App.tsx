  ...
  return <div className="app" onFocusCapture={e => { if (e.target instanceof HTMLInputElement || e.target instanceof HTMLSelectElement) beginGroup(); }} onBlur={e => { if (e.target instanceof HTMLInputElement || e.target instanceof HTMLSelectElement) endGroup(); }}>
    ...
    <main className="workspace">
      ...
      <aside className="inspector">
        ...
        {object && <><div className="inspector-divider" /><div className="subsection-title">{t('inspector.visibility')}<span>{t(isObjectVisible(object, time, project.duration) ? 'inspector.visible' : 'inspector.hidden')}</span></div>
          <NumberField label={t('inspector.visibilityStart')} value={objectRange(object, project.duration).start} min={0} max={objectRange(object, project.duration).end - 0.001} step={1 / project.fps} onChange={start => changeRange(object.id, { ...objectRange(object, project.duration), start })} />
          <NumberField label={t('inspector.visibilityEnd')} value={objectRange(object, project.duration).end} min={objectRange(object, project.duration).start + 0.001} max={600} step={1 / project.fps} onChange={end => changeRange(object.id, { ...objectRange(object, project.duration), end })} />
          <button className="preset-apply" onClick={() => { setPlaying(false); setProject(p => ({ ...p, objects: p.objects.map(o => o.id === object.id ? { ...o, visibility: undefined } : o) })); }}>{t('inspector.fullScene')}</button>
          <p className="microcopy">{t('inspector.visibilityHelp')}</p></>}
        {camera && <><div className="inspector-divider" /><div className="subsection-title">{t('inspector.cameraRange')}<span>{t(liveCamera?.id === camera.id ? 'inspector.live' : 'inspector.standby')}</span></div><NumberField label={t('inspector.cutStart')} value={camera.range.start} min={0} max={camera.range.end - 0.001} step={1 / project.fps} onChange={start => changeRange(camera.id, { ...camera.range, start })} /><NumberField label={t('inspector.cutEnd')} value={camera.range.end} min={camera.range.start + 0.001} max={600} step={1 / project.fps} onChange={end => changeRange(camera.id, { ...camera.range, end })} /><p className="microcopy">{t('inspector.cameraRangeHelp')}</p></>}
