...
test('camera field of view is keyed, interpolated, undoable and saved without motion metadata', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Standard 50°' }).click(); await expect(slider).toHaveValue('50');
  await page.keyboard.press('Control+z'); await expect(slider).toHaveValue('75');
  await page.getByRole('button', { name: 'Camera 01 key at 10.00 seconds', exact: true }).click(); await expect(slider).toHaveValue('25');
...
test('camera tracks hard-cut, earlier starts win overlaps, and gaps render black', async ({ page }) => {
  ...
  await scrub(.3); await page.getByRole('button', { name: 'Add camera at current time', exact: true }).click();
