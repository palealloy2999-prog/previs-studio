...
test('scene editing, playback, JSON round trip, GLB instances and missing assets', async ({ page }) => {
  ...
  await expect(page.getByRole('textbox', { name: 'オブジェクト名' })).toHaveValue('Sphere 1');
  const x = page.getByRole('spinbutton', { name: '位置 X', exact: true });
  ...
  await page.getByRole('button', { name: '先頭に戻る' }).click(); await expect(x).toHaveValue('3');
  await page.getByRole('button', { name: '複製', exact: true }).click(); await expect(x).toHaveValue('4');
  ...
  await page.getByRole('button', { name: '停止', exact: true }).click(); await expect(page.locator('.timecode')).toHaveText('00:00:00');
  ...
  await page.getByRole('button', { name: '色 #7db9ce', exact: true }).click();
  await expect(page.getByRole('alert')).toHaveCount(0);
  const saveEvent = page.waitForEvent('download'); await page.getByRole('button', { name: '保存', exact: true }).click(); const saved = await saveEvent;
...
test('keyed camera FOV and real H.264 MP4 export', async ({ page }) => {
  ...
  await page.getByRole('button', { name: '先頭に戻る' }).click();
  const downloadEvent = page.waitForEvent('download'); await page.getByRole('button', { name: 'MP4を書き出す' }).click();
...
test('visibility trims affect preview and encoded frames and survive JSON reload', async ({ page }) => {
  ...
  const start = page.getByRole('spinbutton', { name: '表示開始（秒）' });
  const end = page.getByRole('spinbutton', { name: '表示終了（秒）' });
  await start.fill('1'); await start.press('Enter'); await end.fill('2'); await end.press('Enter');
  await expect(page.getByText('範囲外・非表示', { exact: true })).toBeVisible();
  const preview = page.locator('.preview-canvas canvas');
  const before = await preview.screenshot();
  await page.getByRole('button', { name: 'Timed Box キー 1.00秒', exact: true }).click();
  ...
  await page.getByRole('button', { name: 'Timed Box キー 2.00秒', exact: true }).click();
  await expect(page.getByText('範囲外・非表示', { exact: true })).toBeVisible();
  ...
  const handle = page.getByRole('button', { name: 'Timed Box 表示開始をドラッグ' });
  ...
  const saveEvent = page.waitForEvent('download'); await page.getByRole('button', { name: '保存', exact: true }).click();
  ...
  const videoEvent = page.waitForEvent('download'); await page.getByRole('button', { name: 'MP4を書き出す' }).click();
...
test('Delete, Undo and Redo preserve edits and group timeline drags', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Character A キー 5.00秒', exact: true }).click();
  ...
  await page.getByRole('button', { name: '元に戻す', exact: true }).click(); await expect(keys).toHaveCount(3);
  await page.getByRole('button', { name: 'やり直す', exact: true }).click(); await expect(keys).toHaveCount(2);
  await page.keyboard.press('Control+z'); await expect(keys).toHaveCount(3);
  const name = page.getByRole('textbox', { name: 'オブジェクト名' });
  ...
  await page.getByRole('button', { name: '元に戻す', exact: true }).click(); await expect(name).toHaveValue('Character A');
  ...
  const handle = page.getByRole('button', { name: 'Character A 表示開始をドラッグ' });
  const box = await handle.boundingBox(); const tracks = await page.locator('.tracks').boundingBox();
  await page.mouse.move(box!.x + box!.width / 2, box!.y + box!.height / 2); await page.mouse.down();
  ...
  await page.getByRole('button', { name: 'Camera 01 キー 0.00秒', exact: true }).click();
  await page.keyboard.press('Delete'); await expect(page.locator('.tracks .track').last().locator('.keyframe')).toHaveCount(1);
  await expect(page.getByRole('status')).toContainText('最後のキーフレーム');
