...
test('scene multi-selection applies transform deltas and copies, pastes, deletes and undoes entities', async ({ page }) => {
  ...
  const x = page.getByRole('spinbutton', { name: '位置 X', exact: true }); await x.fill('1'); await x.press('Enter');
  const ry = page.getByRole('spinbutton', { name: '回転 ° Y', exact: true }); await ry.fill('45'); await ry.press('Enter');
...
test('timeline multi-selection copies, pastes and deletes keys at the playhead', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Character A キー 0.00秒', exact: true }).click();
  await page.getByRole('button', { name: 'Character B キー 0.00秒', exact: true }).click({ modifiers: ['Shift'] });
  ...
  await expect(page.getByRole('button', { name: 'Character A キー 2.00秒', exact: true })).toBeVisible();
  await expect(page.getByRole('button', { name: 'Character B キー 2.00秒', exact: true })).toBeVisible();
  await page.keyboard.press('Delete');
  await expect(page.getByRole('button', { name: 'Character A キー 2.00秒', exact: true })).toHaveCount(0);
  await expect(page.getByRole('button', { name: 'Character B キー 2.00秒', exact: true })).toHaveCount(0);
});
