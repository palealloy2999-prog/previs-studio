...
オブジェクトおよびグループの開始キーから次のキーまでを、編集可能なモーションクリップとして定義できる。

```text
Preset: Bezier / Arc / Barrel Roll
Control Point 1: XYZ
Control Point 2: XYZ
Follow Path Direction: true / false
Roll: degree
```

位置は2つの制御点を持つ3次Bezier曲線から実行時に計算し、中間キーフレームは生成しない。プリセット適用後も制御点を編集できる。編集ビューには軌道と2個のハンドルを表示し、ハンドルは移動ギズモまたは数値入力で調整する。タイムラインには開始キーから終了キーまでを1本のモーションクリップとして表示する。

複数パーツで構成した飛行機などはグループの親キーへモーションパスを設定する。Barrel Rollは進行方向への追従とロール角360度を初期値とし、グループ内の形を維持したまま共通軸で回転する。モーション未設定のキー間は従来の位置・回転補間を使用する。
